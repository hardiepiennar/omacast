...
class DiscoveryTest(unittest.TestCase):
    ...
    def ready_runner(args, *, timeout=5.0):
        ...
        if Path(args[0]).name == "omarchy-cast-guard":
            return CommandResult(tuple(args), 0, json.dumps({"schemaVersion": 1, "kind": "omarchy-cast-guard-version", "apiRevision": 16}), "")
        if args[0] == "nmcli":
    ...
    def test_readiness_proves_the_complete_supported_path(self) -> None:
        ...
        self.assertEqual(guard["apiRevision"], 16)
    ...
    def test_readiness_requires_a_closed_typed_guard_version_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.install_fake_helpers(helpers)
            for payload in (
                {"schemaVersion": True, "kind": "omarchy-cast-guard-version", "apiRevision": 16},
                {"schemaVersion": 1, "kind": "omarchy-cast-guard-version", "apiRevision": 16, "extra": True},
            ):
