...
class ManifestTest(unittest.TestCase):
    ...
    def test_marketplace_payload_contains_omacast_controller(self) -> None:
        ...
        self.assertIn("property bool scanRunning: false", panel)
        self.assertIn("property bool connectAfterScan: false", panel)
        self.assertIn("startPending || connectAfterScan", panel)
        ...
        self.assertIn("root.applyScan(line)", panel)
        self.assertIn('result.kind !== "receiver-discovery"', panel)
        self.assertIn('message = "Receiver scan returned invalid data"', panel)
        self.assertIn('message = items.length ? "Choose a display · still looking…"', panel)
        self.assertIn("connectAfterScan = true", panel)
        ...
        self.assertIn("if (phase !== previousPhase) restorePanelFocus()", panel)
        ...
        self.assertIn("function boundedNumber(value, minimum, maximum, fallback)", panel)
        self.assertIn("function boundedInteger(value, minimum, maximum, fallback)", panel)
        self.assertIn('value.schemaVersion !== 1', panel)
        self.assertIn('message: "Casting state uses an incompatible protocol"', panel)
        ...
        self.assertIn('write(root.pendingPairingPin + "\\n")', panel)
        self.assertNotIn('"--pairing-pin",', panel)
